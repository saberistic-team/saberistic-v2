...
function validateModelGiftIdea(value: unknown, budget: GiftBudgetId): ModelGiftIdea | null {
  if (
    !isRecord(value) ||
    !hasExactKeys(value, [
      'category',
  ...
  if (!sourceUrl || price < range.minimumCents || price > range.maximumCents) return null
          ...
          ideas: {
            items: {
              ...
              properties: {
                ...
                retailer: { maxLength: 80, minLength: 2, type: 'string' },
                sourceUrl: { maxLength: 500, type: 'string' },
