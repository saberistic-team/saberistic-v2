...
- The restricted role has no superuser, database-creation, role-creation, replication, row-security-bypass, inheritance, or cross-role membership capability. It is limited to ten connections and receives only `CONNECT` plus the required rights in `umami`. Before reusing a role, the wrapper fails closed if the role participates in membership in either direction; owns a database, extension, global object, or object outside `umami` (except TOAST objects tied to its `umami` tables); has an explicit database, schema, relation, column, routine, type, or global-object grant outside the expected boundary; has cross-schema default privileges; or has unexpected role/database settings. This materially limits database privileges but cannot isolate CPU, memory, storage, privileges inherited from PostgreSQL's implicit `PUBLIC` role, or catalog state in another database that is not visible from the connected database.
...
| `UMAMI_DATABASE_PASSWORD`          | Render-generated secret                          | Stable password for `saberistic_umami`; used to construct the child URLs and removed as a standalone child variable        |
...
5. Verify in PostgreSQL that the Umami tables are owned by `saberistic_umami`, that the role has no elevated flags, memberships, cross-schema ownership, explicit grants, or default grants, and that it cannot read Payload tables in `public`. A deliberately contaminated staging-role test must stop before the HTTP listener, while an unchanged second start must remain healthy. Verify the database timezone is UTC and perform the acceptance tests below.
