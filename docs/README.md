...
| [33 — Harness Platform M9–M10 workspace adapters](./33-harness-local-docker-workspace-adapters-m9-m10.md)             | Explicit local trust, Docker-by-default copied workspaces, lifecycle evidence, live isolation checks, and integration limits.      |
...
Build Note 013 is live from Harness Platform M6 merge `98924a6`. It documents the additive
...
not a benchmark, and the release is described as checks-gated rather than peer-reviewed. Website
commit `f358e2f` passed CI run `33456489269` and CodeQL run `33456484939`; checks-gated Static Site
deploy `dep-dab24tuq1p3s73fdidm0` generated 32 pages with 13 Build Notes and five Payload prototype
routes, then passed custom-domain article, structured-data, index, feed, sitemap, CDN-cache,
...
across 42 files, strict TypeScript, one golden scenario, and a 19-path pre/post gate with zero
violations before merge; exact-merge CI and CodeQL then passed independently. A clean publication
...
PR-head and exact-merge CI and CodeQL passed, the branch gate retained 693-test evidence with zero
