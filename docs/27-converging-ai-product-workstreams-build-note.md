...
| Gift Draft                 | Yes: quick three-round game, durable real-retailer product inventory, locally cached descriptions and images, asynchronous replenishment/revalidation jobs, signed contribution, reservation lifecycle, payment status, webhook/refund processing, and private fulfillment record are present in the current release candidate. | Partly. Earlier adapters and their safe rollback deployed, but the new cached-inventory architecture has not yet been verified on an exact hosted release.           | No. `GIFTING_AI_ENABLED=0` and `GIFTING_CHECKOUT_ENABLED=0` remain intentional. Gift AI still needs migration, worker, inventory, draw, Redis, bounded-log, and rollback acceptance; Checkout separately needs Stripe and inventory-lifecycle acceptance.                   |
...
Render Static Site UI
  ...
  └─ Gift Draft choices ────────────────> fast deal from durable product inventory
                                         ├─> nine cached retailer products + signed quotes
                                         └─> best-effort minimum-stock job enqueue

Gift inventory worker
  ├─ OpenRouter-grounded discovery ─────> deterministic product checks
  │                                      └─> cached retailer description + product image
  └─ periodic source revalidation ──────> refresh or retire future inventory
...
allows the model to rewrite only bounded explanation fields. Gift Draft uses OpenRouter-grounded
research only in its background inventory pipeline; a player draw does not make a model or retailer
request. The inventory worker validates product structure and policy, fetches the selected retailer
description and image, and stores a local copy before the item can be dealt. Periodic source checks
refresh or retire future inventory without making a temporary retailer failure block an existing
cached draw. Diagnostic fulfillment does not call OpenRouter at all; it authenticates the already
completed report.
...
- A three-round interaction: the server deals nine eligible inventory items, the visitor sees three
  ...
  does not wait for OpenRouter, retailer HTML, or remote product media.
- Durable PostgreSQL inventory stores real products from actual retailers. Each record includes the
  retailer and product URL, observed USD price, product description, suitability note, themes,
  validation state, source timestamps, lifecycle state, and a copied retailer product image.
- The product description and image are fetched and saved on Saberistic's server before an item can
  enter a deal. The game renders that cached copy and links to the original retailer page; there is
  no generated product artwork.
...
- Background discovery uses OpenRouter-grounded product research to find real retailer products,
  then applies deterministic schema, HTTPS URL, budget, product-policy, price-band, and uniqueness
  checks. Model output is a discovery lead, not sufficient by itself: the product description and
  image must be fetched and cached before the record becomes eligible.
- Separate background validation jobs periodically revisit non-sold source pages and media to refresh
  availability, price, description, image, and checked time. A temporary or ambiguous retailer
  failure is recorded for retry and does not break a draw that already has a usable cached copy. A
  product confirmed unavailable, unsafe, or materially mismatched is retired from future deals.
...
- Each deal still enforces the selected budget, exact public response shape, distinct product names
  and source URLs, and—for a mixed deck—low, middle, and high price coverage. The server does not
...
- Prohibited product types are excluded in the model instruction and deterministically rejected
  across the stored product fields and decoded product URL after Unicode normalization and
...
- Displayed availability and price are the most recently observed retailer facts and may change
  between validation runs. Stripe creates a fixed gift contribution to Saberistic, not a purchase
  from the displayed retailer. AmirSaber manually decides what to purchase and may use the
  contribution toward the suggested item, related costs, or a similar substitute.
...
The product owner subsequently approved the durable-inventory design recorded here. Real retailer
products, descriptions, and images are discovered and copied in the background; the player receives
a fast cached deal. Each draw may enqueue future stock, and periodic source checks improve the next
deals without turning a transient retailer failure into a current-draw outage. There is no generated
artwork and no claim that the contribution places a retailer order.

Gift AI remains off. The cached-inventory release candidate has not yet established exact hosted
evidence. Its migration, background discovery, source/media cache, periodic revalidation, stock-floor
replenishment, fast nine-record draw, Redis limits, bounded logs, and enable-to-disable rollback must
pass before `GIFTING_AI_ENABLED=1` can remain on.
...
- `GET /api/gifts/artwork/:id` serves the locally stored retailer product image with a bounded cache
  policy; it does not proxy the retailer on each card render.
- Background inventory jobs discover, cache, and periodically revalidate retailer products without
...
- `GIFTING_AI_ENABLED=1` requires Redis, a separate rate-limit secret, a quote-signing secret, the
  ...
  policy, the durable inventory migration, and a working background job runner. The exact commit must
  pass the complete automated suite, GitHub CI and CodeQL, a flags-off Render deployment, minimum-stock
  discovery with real retailer records and locally cached images/descriptions, periodic refresh and
  retirement behavior, a fast hosted nine-item deal during a simulated retailer failure, same-token
  and concurrency limiter checks, bounded privacy-safe telemetry review, and an enable-to-disable
  rollback drill.
...
1. **Value before capture.** Readiness returns the complete self-serve result before diagnostic
   lead collection. Gift Draft provides the game and cached retailer-product context before Checkout.
...
search had no compatible endpoint. The approved replacement moves grounded discovery, retailer
fetches, image/description caching, and revalidation to durable background jobs and deals only from
cached inventory. A bounded, process-local single-flight drain runs those durable jobs after status
and draw responses on the existing Render web service; this avoids a new paid worker while keeping
OpenRouter and retailer latency out of the player response. A standalone worker command remains for
supervised prefill or a future dedicated service. That release still needs exact-deploy, migration,
inventory, hosted draw, Redis, log, and rollback gates. Payment/refund/fulfillment acceptance also remains open, so
`GIFTING_AI_ENABLED=0` and `GIFTING_CHECKOUT_ENABLED=0` are still intentional.

1. Configure the pinned Gift model and run an opt-in real OpenRouter background-discovery smoke under
   the existing spend and provider/privacy controls. Confirm it produces real retailer URLs and that
   the server copies the product descriptions and retailer images before eligibility.
2. Push the exact cached-inventory commit, wait for CI and CodeQL, deploy it to Render with both Gift
   flags still `0`, apply the inventory migration, and verify the request-tail job drain without
   provisioning a paid worker.
3. Fill the minimum inventory and inspect a bounded sample for safe product identity, distinct source
   URLs, price/budget policy, locally served images, copied descriptions, and checked timestamps.
4. Temporarily enable AI for a supervised canary. Confirm a hosted draw returns exactly nine cached
   real-retailer products without waiting on OpenRouter or the retailer, queues replenishment when
   stock is below the floor, survives a simulated source/media outage, exercises the same-token Redis
   success-to-`429` boundary and concurrency behavior, emits bounded logs, and can return to `0`.
5. Exercise periodic revalidation: successful checks refresh availability, price, description, image,
   and checked time; ambiguous failures retry without breaking an already delivered cached deck; and
   a confirmed unavailable item is removed from future deals.
...
| Unit/integration suite         | Passing/skipped totals for `pnpm test:int`                                                                                                                                | Historical: 660 passed and 4 opt-in live tests skipped across 47 files for the now-superseded mandatory retailer-fetch release candidate. Record a fresh total for the cached-inventory, job, artwork, and reservation contract before release.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| OpenRouter Gift adapter        | Background grounded discovery of real retailer products followed by local description/image caching                                                                       | Pending for the current contract. Historical September 1 experiments showed that the deprecated web plugin plus strict output returned nine records without annotations and that server-tool search plus the same strict schema had no compatible endpoint. That evidence rules out the former request-time design; it does not prove the new background discovery, cache, or revalidation pipeline.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Gift hosted adapter history    | Exact commit/deploy, migrated inventory, active worker, cached nine-record draw, replenishment/revalidation, disabled-state verification, rate limits, logs, and rollback | Commit `69037dfa4f82f05642d8cfd61818cb6d4db0e059` passed CI and CodeQL and reached web deploy `dep-dabjign7e77c73ac0dgg` and Static Site deploy `dep-dabjigh5efls73anqovg`. Temporary canary `dep-dabjjpbm8hqs73e31oog` returned nine cards, but the then-required page review found one out-of-stock item and four price disagreements; rollback `dep-dabjlh3m8hqs73e37qu0` restored both flags false and the disabled `503`. The superseded mandatory request-time retailer verifier later failed two hosted draws: under-$30 checked 21 and verified four (`load: 8`, `budget_above: 6`); mixed checked 46 and verified six (`load: 26`). The current cached-inventory architecture has not yet passed its exact-artifact hosted gates.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Rollback/deactivation          | Evidence that all four provider flags can be returned to `0` without removing deterministic readiness                                                                     | Gift AI's failed canary was returned to `0` by `dep-dabjlh3m8hqs73e37qu0`; status and disabled-route checks passed afterward. Readiness AI remains enabled and accepted, while `GIFTING_AI_ENABLED=0`, `GIFTING_CHECKOUT_ENABLED=0`, and `DIAGNOSTIC_ENABLED=0` preserve the other provider boundaries. The cached-inventory Gift release still needs its own enable-to-disable drill after its hosted canary; a live readiness enable-to-disable drill also remains pending.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

### Cached real-product inventory release candidate — September 1, 2026

The final product decision from the Gift Draft workstream is to keep the game quick while retaining
real retailer references. OpenRouter researches product ideas off the player path. The server then
accepts only approved retailer URLs, fetches the retailer's canonical product data and image, checks
the price band, product identity, and deterministic exclusions, converts the image to a bounded
WebP, and stores the description and image bytes in PostgreSQL. A draw reads only cached metadata;
it neither calls OpenRouter nor fetches a retailer. Missing availability metadata is treated as
unknown rather than invented. Temporary retailer/network validation failure makes an item stale and
retryable while a bounded cached copy remains drawable; explicit unavailability, unsafe identity,
or a deterministic policy failure retires it. A paid contribution will remove the suggestion from
available inventory, but the checkout flag remains independent because the charge is a fixed gift
contribution rather than a retailer order.

The durable discovery and revalidation queue is drained after successful status and draw responses,
at most three jobs per status pass and one per draw pass, with one active drain per process. This
keeps the existing Render web service and avoids silently adding a paid worker. Every status pass
checks the four concrete price lanes, enqueues missing baseline stock, queues due revalidation, and
prunes old job/image data. The browser distinguishes paused from restocking and retries restocking
status after one minute. A standalone worker command is retained for a supervised prefill or a
future paid worker.

Local acceptance for this exact architecture passed before deployment: the full ordinary suite
reported 634 passing tests and three intentionally skipped opt-in tests; root and static-site
typechecks, lint, the Payload production build, and the 29-page fixture export passed. The opt-in
live worker canary made two pinned `openai/gpt-4.1-mini` OpenRouter calls through the current
server-side web-search tool, selected a real No Starch Press product, fetched the retailer page and
image through DNS-pinned connections, preserved its observed $49.99 price in the correct inventory
lane, normalized a 43,992-byte WebP, and reached the cache insert boundary without exposing the key
or raw model output. An isolated PostgreSQL 16.14 acceptance run applied all ten migrations
after adding the administrator required by the older prototype-publication migration, verified the
new inventory/job schema and constraints, proved two concurrent reservations have exactly one
winner, and proved a sold item disappears from later deals while its cached image remains intact.
The current Render database already has administrators, so that older fresh-database bootstrap
constraint does not block this upgrade; it remains a clean-install runbook gap.
...
3. deploy the exact cached-inventory Gift commit only after CI and CodeQL pass; apply the migration,
   exercise the bounded request-tail drain, fill the minimum stock with real retailer records whose descriptions
   and images are copied locally, and prove periodic checks can refresh or retire future inventory.
   Then prove a hosted draw returns exactly nine eligible cached products without waiting on
   OpenRouter or a retailer, queues replenishment below the stock floor, survives a simulated source
   failure, confirms the same anonymous token receives four allowed draws followed by `429`, exercises
   IP/daily/concurrency failure behavior, reviews bounded cost/privacy logs, and completes an
   enable-to-disable rollback drill before `GIFTING_AI_ENABLED=1` can remain set;
