...
| Gift Draft                 | Yes: three-round game, citation-checked OpenRouter search, signed quote, fixed contribution Checkout, payment status, webhook/refund processing, and private fulfillment record are present. | Yes: the game and APIs are deployed, its private schema migrated, the page passes hosted smoke, and the disabled ideas route returns `503`.                          | No. The OpenRouter adapter passed a local-key live search, but public AI remains off pending its separate deployed search, limiter, and provider-policy acceptance; Checkout remains off pending Stripe fulfillment gates.                                                  |
...
Checkout-Sessions-only restricted key, event-scoped snapshot webhook, and corresponding Render
secrets are provisioned. Resend, verified-sender, booking, payment, fulfillment, and retention
acceptance remain open, so `DIAGNOSTIC_ENABLED=0` is still intentional.
...
key, five-event snapshot webhook, and corresponding Render secrets are provisioned. The live Gift
AI draw and payment/refund/fulfillment acceptance remain open, so `GIFTING_AI_ENABLED=0` and
...
| Unit/integration suite         | Passing/skipped totals for `pnpm test:int`                                                                 | 539 passed and 3 opt-in live tests skipped across 44 files.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| OpenRouter Gift adapter        | One real cited nine-item search using the local key                                                        | An earlier smoke passed with `openai/gpt-4.1`: nine validated ideas, 24 citations, 9,919 tokens, four executed server-tool steps, and $0.041726. Revalidation under the final Guardrail reached both pinned models and failed closed: the primary returned one out-of-range price, while the fallback returned noncanonical item keys. Gift AI remains off pending a fully validated current-policy draw.                                                                                                                                                    |
...
3. make Gift Draft AI pass its separate deployed search, limiter, and provider-policy acceptance
   before `GIFTING_AI_ENABLED=1`;
