...
| Gift Draft                 | Yes: quick three-round game, durable model-generated concept inventory, locally cached generated WebP images, asynchronous replenishment jobs, signed contribution, reservation lifecycle, payment status, webhook/refund processing, and private fulfillment record are present in the current release candidate. | Partly. Earlier retailer-backed adapters and their safe rollback deployed, but the generated-concept inventory architecture has not yet been verified on an exact hosted release. | No. `GIFTING_AI_ENABLED=0` and `GIFTING_CHECKOUT_ENABLED=0` remain intentional. Gift AI still needs migration, worker, inventory, draw, Redis, bounded-log, image-cost, and rollback acceptance; Checkout separately needs Stripe and inventory-lifecycle acceptance.       |
...
Gift AI remains off. The generated-inventory release candidate has not yet established exact hosted
evidence. Its cutover migration, structured concept generation, base64 image validation and WebP
cache, stock-floor replenishment, fast nine-record draw, Redis limits, bounded logs, image spend, and
enable-to-disable rollback must pass before `GIFTING_AI_ENABLED=1` can remain on.
...
- `GIFTING_AI_ENABLED=1` requires Redis, a separate rate-limit secret, a quote-signing secret, the
  ...
  pass the complete automated suite, GitHub CI and CodeQL, a flags-off Render deployment, the legacy
  inventory cutover, minimum-stock generation with bounded cached WebP images, a fast hosted nine-item
  deal with no provider call on the player path, same-token and concurrency limiter checks, bounded
  privacy-safe telemetry and spend review, and an enable-to-disable rollback drill.
...
A standalone worker command remains for supervised prefill or a future dedicated service. That
release still needs exact-deploy, migration, inventory, hosted draw, Redis, image-cost, log, and
rollback gates. Payment/refund/fulfillment acceptance also remains open, so `GIFTING_AI_ENABLED=0`
and `GIFTING_CHECKOUT_ENABLED=0` are still intentional.
...
| OpenRouter account gates       | Hard spend cap, plugin override review, logging/data-use settings, guardrail, pinned models, ZDR endpoints                                                                      | Passed for readiness and the Gift canary. The active key-specific Guardrail enforces a $5 monthly cap, ZDR, prompt-injection `Flag`, all eight sensitive-information presets, **Only Allow** providers Azure and Google Vertex, and **Only Allow** models `google/gemini-2.5-flash-lite`, `openai/gpt-4.1-mini`, `openai/gpt-4.1`, and `google/gemini-3.1-flash-lite-image`. The key separately retains its $10 total cap. Paid-model training, prompt storage, broadcast, Response Healing, and Pareto Router defaults are off. The global free-training toggle is on, but no free or suffixed model is allowlisted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Gift hosted adapter history    | Exact commit/deploy, legacy cutover, active worker, cached nine-record generated draw, replenishment, disabled-state verification, rate limits, logs, image spend, and rollback | Commit `69037dfa4f82f05642d8cfd61818cb6d4db0e059` passed CI and CodeQL and reached web deploy `dep-dabjign7e77c73ac0dgg` and Static Site deploy `dep-dabjigh5efls73anqovg`. Temporary canary `dep-dabjjpbm8hqs73e31oog` returned nine cards, but the then-required page review found one out-of-stock item and four price disagreements; rollback `dep-dabjlh3m8hqs73e37qu0` restored both flags false and the disabled `503`. The superseded mandatory request-time retailer verifier later failed two hosted draws: under-$30 checked 21 and verified four (`load: 8`, `budget_above: 6`); mixed checked 46 and verified six (`load: 26`). The current generated-concept inventory architecture has not yet passed its exact-artifact hosted gates.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Rollback/deactivation          | Evidence that all four provider flags can be returned to `0` without removing deterministic readiness                                                                           | Gift AI's failed canary was returned to `0` by `dep-dabjlh3m8hqs73e37qu0`; status and disabled-route checks passed afterward. Readiness AI remains enabled and accepted, while `GIFTING_AI_ENABLED=0`, `GIFTING_CHECKOUT_ENABLED=0`, and `DIAGNOSTIC_ENABLED=0` preserve the other provider boundaries. The generated-concept Gift release still needs its own enable-to-disable drill after its hosted canary; a live readiness enable-to-disable drill also remains pending.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

### Generated-concept inventory release candidate — September 1, 2026
...
experiments above remain useful historical evidence but do not satisfy this release candidate's
hosted migration, inventory, draw, and rollback gates.
...
3. deploy the exact generated-inventory Gift commit only after CI and CodeQL pass; apply the cutover
   migration, prove unsold retailer rows and validation jobs are retired without deleting sold
   history, exercise the bounded request-tail drain, and fill every price lane with distinct generated
   concepts whose images are copied into the local WebP cache. Then prove a hosted draw returns
   exactly nine eligible cached concepts without waiting on OpenRouter, queues replenishment below the
   stock floor, rejects malformed or oversized image output, confirms the same anonymous token
   receives four allowed draws followed by `429`, exercises IP/daily/concurrency failure behavior,
   reviews bounded cost/privacy logs and OpenRouter image spend, and completes an enable-to-disable
   rollback drill before `GIFTING_AI_ENABLED=1` can remain set;
